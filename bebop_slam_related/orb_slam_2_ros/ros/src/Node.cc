...
void Node::Init () {
  ...
  node_handle_.param<std::string>(name_of_node_+ "/pointcloud_frame_id", map_frame_id_param_, "bebop/map");
  ...
  //
  points_pix_3d_publisher_ = node_handle_.advertise<orb_slam_2_ros_msgs::PointPix3ds> (name_of_node_+"/point_pix_3d", 1);
}
...
void Node::Update () {
  ...
  // vector<bool> vbMap = orb_slam_->mpFrameDrawer->mvbMap;
  vector<std::pair<cv::Point, cv::Point3f>> PointsPix3d = orb_slam_->mpFrameDrawer->PointsPix3d;
  // vector<cv::Point> currentIP = orb_slam_->mpFrameDrawer->currentIP;
  // size_t N = orb_slam_->mpFrameDrawer->mvCurrentKeys.size();
  // vector<cv::KeyPoint> vCurrentKeys = orb_slam_->mpFrameDrawer->mvCurrentKeys;
  orb_slam_2_ros_msgs::PointPix3ds points;
  size_t N = PointsPix3d.size();
  // std::cout << "N: " << N << std::endl;
  for (size_t i = 0; i < N; i++)
  {
    // std::cout << "?" << vbMap[i] << std::endl;
    orb_slam_2_ros_msgs::PointPix3d one;
    one.pointPix.x = PointsPix3d[i].first.x;
    one.pointPix.y = PointsPix3d[i].first.y;
    one.point3d.x = PointsPix3d[i].second.x;
    one.point3d.y = PointsPix3d[i].second.y;
    one.point3d.z = PointsPix3d[i].second.z;
    points.points.push_back(one);
  }
  points_pix_3d_publisher_.publish(points);
